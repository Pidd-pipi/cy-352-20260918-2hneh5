...
门店在首页「赛事活动」面板发布比赛（选择桌游、开始时间、人数上限、每组人数），玩家报名后页面每 3 秒自动刷新剩余名额；满员后系统按报名顺序轮流分组（组间人数差不超过 1 人）并立即锁定报名，分组结果持久化在 MongoDB 中，刷新页面后仍在。

并发与一致性设计：

- 报名占座使用 `registeredCount < capacity` 条件的原子自增，从数据库层面杜绝超员；
- 报名表对 `(赛事, 玩家昵称)` 建唯一索引，并发重复报名只保留一份有效记录，冲突请求自动回滚名额；
- 满员判定基于真实报名记录数，`open -> locked` 状态翻转是原子条件更新，并发触发时仅一次生效。

赛事 API：

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/tournaments` | 赛事列表（含剩余名额、状态） |
| POST | `/api/tournaments` | 发布赛事（name/game/startTime/capacity/groupSize） |
| GET | `/api/tournaments/:id` | 赛事详情（报名顺序、分组结果） |
| POST | `/api/tournaments/:id/register` | 玩家报名（body: `{ "playerName": "..." }`） |
