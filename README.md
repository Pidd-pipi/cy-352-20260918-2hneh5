...
- 活动赛事发布：门店发布桌游赛事活动（可设桌游、开始时间、人数上限、每组人数），玩家报名后实时看到剩余名额；报名接口为单条原子更新，并发抢最后一席或重复报名只会生效一份，不超员不漏人；满员后按报名顺序轮流分组（任意两组人数差不超过 1），分组完成即锁定报名。

## 赛事活动 API

| 方法 | 路径 | 说明 |
| --- | --- | --- |
| GET | `/api/tournaments` | 赛事列表（含已报人数、剩余名额、状态、分组） |
| POST | `/api/tournaments` | 发布赛事：`name`、`boardGame`、`startTime`、`maxParticipants`(2-128)、`groupSize`(2-人数上限) |
| GET | `/api/tournaments/:id` | 赛事详情 |
| POST | `/api/tournaments/:id/register` | 玩家报名：`player`；重复报名 / 满员 / 已锁定均返回 409 |

前端「赛事活动」页每 2.5 秒轮询刷新名额，数据持久化在 MongoDB `tournaments` 集合，刷新或重启服务后数据仍在。
